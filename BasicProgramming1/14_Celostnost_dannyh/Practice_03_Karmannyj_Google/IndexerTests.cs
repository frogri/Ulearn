...
namespace PocketGoogleTest
{
    ...
    public class Indexer_Tests
    {
        ...
        private readonly Dictionary<int, string> dictionary = new Dictionary<int, string> //данные(файлы) для индексера
        {
            { 0, "A B C" },
            { 1, "B C" },
            { 2, "A C A C" },
        ...
        public void Add() //добавляем файлы в индексер
        {
            ...
            foreach (var d in dictionary) //проходим по всем файлам
                indexer.Add(d.Key, d.Value);    //добавляем файлы в индексер
        ...
        {
            var actual = indexer.GetIds(word);
        ...
        [TestCase(3, "f", new[] { 3 })]
        [TestCase(0, "X", new int[0])] //ищем "Х"
        [Order(02)]
        public void GetPositions(int id, string word, int[] expected)
        {
            var actual = indexer.GetPositions(id, word);
            Assert.AreEqual(expected.ToList(), actual);
        }
        ...
        // Выполнить либо Тест 5, либо Тест 6
        ...
        public void Add_Remove_Add_GetPosition_B(string word, int id, int[] expected)
        {
            indexer.Add(id, word);
            indexer.Remove(id);
            indexer.Add(id, word);                            //добавляем файл 1
            var actual = indexer.GetPositions(id, word);      //ищем слово "В" в файле 1 - "B C"
        ...
        //    indexer.Remove(id);
        //    indexer.Add(id, word);                            //добавляем файл 1
        //    var actual = indexer.GetPositions(id, "A");       //ищем слово "A" в файле 1 - "B C"
